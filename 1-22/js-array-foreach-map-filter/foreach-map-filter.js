...
function vowelCount(str){
    ...
    split.forEach(function(val){
        ...
        if(vowls.indexOf(lowerCaseVal) !== -1){
            //the current letter is a vowel
            if(result[lowerCaseVal]){
                result[lowerCaseVal] ++
            }
            else{
                result[lowerCaseVal] = 1
            }
        }
    })
    return result;
...
function removeVowels(str) {
    const vowels = "aeiou";
    let initArr = str.toLowerCase().split("");
    let resultArr = initArr.filter(function(val) {
